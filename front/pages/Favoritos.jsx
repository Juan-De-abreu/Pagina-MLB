  ...
  const getDatos = async () => {
    ...
    try {
      ...
      const response = await fetch(url, {
        ...
        mode: 'cors', // asegura enviar solicitud CORS
      });
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center py-10">
        <div role="status" aria-label="loading" className="w-10 h-10 border-4 border-red-500 border-t-transparent rounded-full animate-spin" />
        ...
        {datos.length === 0 ? (
          ...
          <div className={`grid gap-4 ${columnasGrid[tipoFiltro]}`}>
            {tipoFiltro === "jugadores"
              ...
              : tipoFiltro === "equipos"
                ? datos.map((item, index) => <CardEquipos
                  key={item.id}
                  item={item}
                  contador={index + 1} />)
                : datos.map((item, index) => <CardPartidos
                  key={item.id}
                  item={item}
                  contadorpartidos={index + 1} />)}
          </div>
        )}
      </div>
    </div>
