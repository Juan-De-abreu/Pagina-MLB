  ...
  useEffect(() => {
    if (token) {
      try {
        ...
        setUser({ nombre: payload.nombre, es_admin: payload.es_admin, id: payload.sub });
      } catch {
        setUser(null);
      }
    }
  ...
  const fetchNotificaciones = async () => {
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(`Error HTTP ${res.status}: ${errorMsg || res.statusText}`);
      }
      ...
      setNotificacionesNuevasCount(0);
      // Opcionalmente mostrar mensaje o manejar error UI
    }
  };
  fetchNotificaciones();
}, [user]);
    ...
    <div className="navbar flex justify-between items-center h-20 lg:h-30 py-auto lg:px-8 m-0 py-0 border-b border-[#494a0c] bg-[var(--vinotinto)] text-[var(--blanco-hielo)]">
      ...
      <div
        ...
        className={`
          ...
          lg:static
          flex-1
          ${menuOpen ? 'z-51 flex flex-col items-center gap-4 py-2 pt-16 md:py-4 bg-linear-to-t to-[#000000df] from-[#000000f3] md:to-[#000000b6] md:from-[#000000cf] rounded-lg' : 'hidden'}
      ...
      >
              ...
              <div key={to} className={`${size} block lg:hidden relative`}>
                {user ? (
                  <>
                    ...
                    >
                      <svg xmlns="http://www.w3.org/2000/svg" className="inline-block w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} >
                        <path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6l3 3" />
                        <circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth={2} fill="none" />
                      </svg>
                    </button>
                    {userMenuOpen && (
                      <div className="absolute top-full mt-2 left-0 w-full bg-[var(--vinotinto)] border border-[var(--dorado)] rounded-xl shadow-lg z-50">
                        <div className="px-4 py-2 text-white font-semibold">{user.nombre}</div>
                        <button
        ...
        <label
          htmlFor="menuopenid"
          className={`hover:scale-120 transition-all duration-200 text-center py-1 md:text-2xl text-1xl ${
      ...
      {/* Menú usuario con rueda y opciones */}
          ...
          <>
            ...
            {userMenuOpen && (
              ...
              >
                ...
                <NavLink
                  to="/notificaciones"
                  className="w-full text-left px-4 py-2 hover:bg-[var(--dorado)] hover:text-black flex justify-between items-center"
                  onClick={() => setUserMenuOpen(false)}
